...
const [openIndex, setOpenIndex] = useState<number | null>(null);
 ...
 setOpenIndex(openIndex === index ? null : index);
    ...
    <div>
       <div className="flex flex-col gap-2 mb-10 justify-center items-center">
        <h2 className="text-2xl md:text-3xl tracking-wide head font-semibold dark:text-white">Common Questions
</h2>
    
        <p className="dark:text-[#cac4cf]">Everything you need to know about the EduCert experience.</p>
        </div> 
 ...
 <div
 key={index}
 className="bg-white dark:bg-[#1b2337]  rounded-2xl p-5 md:p-8 shadow-sm"
 >
 <button
 onClick={() => handleToggle(index)}
 className="w-full flex justify-between items-center text-left"
 >
 <h3 className="font-semibold text-xl md:text-2xl dark:text-white">{faq.question}</h3>
