...
export default function LessonPage() {
  ...
  const getExerciseData = () => {
    ...
    if (title.includes("variable") || title.includes("basic")) {
      return {
        question:
          "Create a variable named `score` and assign it the integer value `100`:",
    ...
    if (title.includes("function") || title.includes("method")) {
      return {
        question:
          "Define a function header named `calculate` that accepts no arguments:",
  ...
  const markdownRenderers = {
    code({ node, inline, className, children, ...props }: any) {
      ...
      return !inline && match ? (
        ...
        <code
          className={`${className} px-1.5 py-0.5 rounded bg-gray-100 dark:bg-black/30 text-purple-600 dark:text-[#E2FB6C] font-mono text-xs`}
          {...props}
        >
          {children}
        </code>
      ...
      <div className="flex-1 flex overflow-hidden min-h-0">
              ...
              <div className="space-y-3">
                ...
                {exerciseError && (
                  <p className="text-[10px] text-rose-500 font-medium animate-pulse">
                    Incorrect. Please check the syntax or try the hint!
                  </p>
                )}
              </div>
            </div>
        ...
        <section className="w-120 border-l border-gray-200/80 dark:border-white/5 bg-gray-900 dark:bg-[#081816] flex flex-col justify-between lg:flex shrink-0">
          <div className="h-12 border-b border-gray-800/80 dark:border-white/5 px-4 flex items-center justify-between text-xs text-gray-400 font-mono">
            ...
            >
              {isRunning ? (
                <MdCached className="animate-spin" size={14} />
              ) : (
                <MdPlayArrow size={14} />
              )}
              Run Code
