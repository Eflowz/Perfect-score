  ...
  return (
    <header className="w-full h-16 border-b border-gray-200/80 dark:border-white/5 bg-white/80 dark:bg-gray-950/80 backdrop-blur-md sticky top-0 z-30 flex items-center justify-between px-8 transition-colors duration-200">
      ...
      <div className="flex items-center space-x-2 text-xs font-mono tracking-wide">
        <span className="text-gray-400 dark:text-[#6B8A85] hover:text-[#16423C] dark:hover:text-white cursor-pointer transition-colors">Dashboard</span>
        <span className="text-gray-300 dark:text-white/20">/</span>
        <span className="text-gray-400 dark:text-[#6B8A85] hover:text-[#16423C] dark:hover:text-white cursor-pointer transition-colors">Python Pro</span>
        <span className="text-gray-300 dark:text-white/20">/</span>
        <span className="text-[#16423C] dark:text-[#E2FB6C] font-semibold">Lesson_12</span>
      </div>
      ...
      <div className="flex items-center space-x-4 sm:space-x-5">
        ...
        <div className="relative hidden lg:block group">
          <MdSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-[#16423C] dark:group-focus-within:text-[#E2FB6C] transition-colors" size={18} />
          <input 
            type="text" 
          ...
          >
            ...
            <div className="flex-col space-y-0.5 hidden sm:block">
              <span className="text-xs font-bold text-gray-900 dark:text-white leading-none tracking-tight flex items-center gap-1">
                {getUserDisplayName()}
                <MdKeyboardArrowDown size={14} className={`text-gray-400 transition-transform duration-200 ${dropdownOpen ? "rotate-180" : ""}`} />
              </span>
          ...
          {dropdownOpen && (
            <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-[#16423C] border border-gray-200 dark:border-white/10 rounded-xl shadow-xl p-1.5 z-50 animate-in fade-in slide-in-from-top-2 duration-100">
              <div className="px-2.5 py-2 border-b border-gray-100 dark:border-white/5 sm:hidden">
                <p className="text-xs font-bold text-gray-900 dark:text-white">{getUserDisplayName()}</p>
                <p className="text-[9px] font-mono text-gray-400 dark:text-[#6B8A85] mt-0.5">Level {xpMetrics.level} • {xpMetrics.xpDisplay} XP</p>
              </div>
